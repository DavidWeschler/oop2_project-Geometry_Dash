...
Robot::Robot(std::unique_ptr<b2World>& world, sf::Vector2f position)
	: Enemy(world, robotColor, position, b2_dynamicBody, sf::Vector2f(1, 2)), m_animation(TexturesManger::instance().animationData(Robot_E), Direction::Right, *this)
{
	m_way = 1;
	setSize({1.5*60, 1.5*60});
}

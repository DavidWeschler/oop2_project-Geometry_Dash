...
{
	m_buttons.push_back(Button(sf::Vector2f((WINDOW_X*4/10.f) * 1.f, (WINDOW_Y*2.5/3) * 1.f), sf::Vector2f(150, 150), &m_resources.getMenuButtonTexture(5),
		std::move(std::make_unique<NextStateCommand>(controller, GameStates::MENU_S)))); //exit to menu

	m_buttons.push_back(Button(sf::Vector2f((WINDOW_X *6/10.f) * 1.f, (WINDOW_Y * 2.5 / 3) * 1.f), sf::Vector2f(150, 150), &m_resources.getMenuButtonTexture(6),
...
void NextLevelWindow::setDisplayMembers()
{
	...
	m_stats.setPosition(sf::Vector2f((WINDOW_X / 3.6f) * 1.f, (WINDOW_Y / 2.4f) * 1.f));
	...
	m_wellDone.setSize(sf::Vector2f((WINDOW_X * 109.f / 200.f) * 1.f, (WINDOW_Y * 19.f / 90.f) * 1.f));
	m_wellDone.setPosition(sf::Vector2f((WINDOW_X / 4.5f) * 1.f, (WINDOW_Y / 7.f) * 1.f));

...
void TexturesManger::LoadFromFile()
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		if (!(m_menusTextures[i].loadFromFile(m_menuTools[i]))) exit(EXIT_FAILURE);
	}

	for (int i = 0; i < 3; i++)
	{
		m_backButtonTextures.push_back(sf::Texture());
		if (!(m_backButtonTextures[i].loadFromFile(m_backButtNames[i]))) exit(EXIT_FAILURE);
	}
	...
	for (int i = 0; i < NUM_OF_MENU_BUTTONS; i++)
	{
		...
		if (!(m_menuButtonTextures[i].loadFromFile(m_menuButtonsNames[i])))exit(EXIT_FAILURE);
	}
	...
	for (int i = 0; i < NUM_OF_PLAYERS; i++)
	{
		...
		if (!(m_playerTextures[i].loadFromFile(m_playerNames[i]))) exit(EXIT_FAILURE);
	}
	...
	for (int i = 0; i < NUM_OF_CHOOSE_SETS; i++)
	{
		...
		if (!(m_setsTextures[i].loadFromFile(m_setsNames[i]))) exit(EXIT_FAILURE);
	}
	...
	for (int i = 0; i < NUM_OF_LEVELS; i++)
	{
		...
		if (!(m_images[i].loadFromFile(m_levelNames[i]))) exit(EXIT_FAILURE);
	}

	if (!(m_cursor.loadFromFile("cursor.png"))) exit(EXIT_FAILURE);
	
	//Load objects
	for (int i = 0; i < NUM_OF_OBJ; i++)
	{
		m_objTextures.push_back(sf::Texture());
		if (!(m_objTextures[i].loadFromFile(m_objNames[i]))) exit(EXIT_FAILURE);
	}
	...
	for (int i = 0; i < NUM_OF_HOW_TO_PLAY; i++)
	{
		...
		if (!(m_howToPlay[i].loadFromFile(m_slides[i]))) exit(EXIT_FAILURE);
	}

	if (!m_howToPlayBackground.loadFromFile("HowToPlayBackground.png")) exit(EXIT_FAILURE);
	
	for (int i = 0; i < URL_BUTTON_NAME; i++)
	{
		m_urlTexture.push_back(sf::Texture());
		if (!(m_urlTexture[i].loadFromFile(m_urlNames[i]))) exit(EXIT_FAILURE);
	}
	...
	for (int i = 0; i < NUM_OF_BULLETS; i++)
	{
		...
		if (!(m_bulletsTex[i].loadFromFile(m_bulletNames[i]))) exit(EXIT_FAILURE);
	}
	...
	for (int i = 0; i < NUM_OF_PROMPTS; i++)
	{
		...
		if (!(m_promptTexture[i].loadFromFile(m_promptsNames[i]))) exit(EXIT_FAILURE);
	}

	//well done
	if (!m_wellDone.loadFromFile("WellDone.png")) exit(EXIT_FAILURE);
	
	//load font
	if (!m_font.loadFromFile("font.ttf")) exit(EXIT_FAILURE);
	
	//statistics background when shown
	if (!m_statsBackground.loadFromFile("statsBackground.png")) exit(EXIT_FAILURE);
	
	//statistics background when not shown
	if (!m_noStatsBackground.loadFromFile("NoStats.png")) exit(EXIT_FAILURE);
